# Lecture: Building a Word Count Pipeline — From Batch Processing to Streaming Foundations
...
This lecture focuses on **steps 1–3**, laying a strong architectural and engineering base before introducing streaming.
...
- **Execution Model:** Spark DAG (Stages → Tasks)
...
BatchWordCount
 ...
 ├── get_quality_data()
 ├── get_word_count()
 ├── overwrite_word_count()
 └── word_count()   # Orchestration method
...
BatchWordCountTestSuite
 ├── clean_tests()
 ├── ingest_data(iteration)
 ├── assert_result(expected_count)
 └── run_tests()
...
- Supports multiple iterations:
    - Iteration 1 → text_data_1.txt
    - Iteration 2 → text_data_2.txt
    - Iteration 3 → text_data_3.txt
...
Job 1: Data ingestion from source → Landing Zone

Job 2: Batch Word Count pipeline

Workflow: Job 1 → Job 2
...
➡️ Next Lecture:
Transforming this batch word count pipeline into a Structured Streaming solution and addressing real-time processing challenges.
